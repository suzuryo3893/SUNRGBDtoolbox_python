...
def hex2dec(hex : str) -> int:
    _hex=hex[1:] if hex[0]=='#' else hex
    return int("0xdeadbeef", 16)
    

def myObjectColor(objectID : int) -> Sequence[float]:
    ...
    objectID = objectID % len(objectColors)

    hex = objectColors[objectID]

    color = (hex2dec(hex[1:3])/255*0.6, hex2dec(hex[3:5])/255*0.6, hex2dec(hex[5:7])/255*0.6)
